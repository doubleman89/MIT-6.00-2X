...
class RectangularRoom(object):
    ...
    def __init__(self, width, height, dirt_amount):
        ...
        """
        if width <= 0: 
            raise ValueError("width should be higher than 0")

        if height <= 0: 
            raise ValueError("height should be higher than 0")
        
        if dirt_amount < 0: 
            raise ValueError("dirt amount can't be negative")

        self.width = width
        self.height = height      
        self.tiles = dirt_amount
    ...
    def tiles(self,value):
        self._tiles = [[value for y in range(self.height)] for x in range(self.width)] 
        ...
        try:
            for x in range(len(self.tiles)):
                for y in range(len(self.tiles[x])):
                    if self.is_tile_cleaned(x,y): count +=1        
        except TypeError:
            print(type(self.tiles), self.tiles)                    
    ...
    def is_position_in_room(self, pos :Position):
        ...
        """
        return math.floor(pos.x) in range(self.width)  and math.floor(pos.y) in range(self.height)
...
class Robot(object):
    ...
    def __init__(self, room :RectangularRoom, speed :float, capacity:int):
        ...
        """
        if speed <= 0.0: 
            raise ValueError("Speed value should be higher than 0")
...
class FurnishedRoom(RectangularRoom):
    ...
    def is_position_furnished(self, pos : Position):
        ...
        return (math.floor(x),math.floor(y)) in self.furniture_tiles
    ...
    def get_random_position(self):
        ...
        while( posOk):
                pos= Position(random.randrange(0,self.width),random.randrange(0,self.height))
                if self.is_position_valid(pos) : break        

        return pos


# fRoom = FurnishedRoom(5,6,10)
# fRoom.add_furniture_to_room()
# fTiles = fRoom.furniture_tiles
# print(fRoom.tiles[4][5]) 

# assert fRoom.is_tile_furnished(fTiles[0][0],fTiles[0][1])==True, "check furnished tiles"
# assert fRoom.is_tile_furnished(fRoom.width,fRoom.height) == False , "assert wrong furnished position"

# print("cleaned num tiles", fRoom.get_num_cleaned_tiles())
# print("num tiles", fRoom.get_num_tiles())



# cleanCapacity = 3
# dirtPos = fRoom.get_random_position()
# dirtPosX = math.floor(dirtPos.x)
# dirtPosY = math.floor(dirtPos.y)
# # create lambda function 
# getFRoomDirt = lambda : fRoom.get_dirt_amount(dirtPosX,dirtPosY)
# # set initial dirt value
# dirt = getFRoomDirt()
# print("Dirt amount on tile ", dirtPosX,dirtPosY," is " ,dirt)

# while(dirt>0):
#     fRoom.clean_tile_at_position(dirtPos,cleanCapacity)
#     print("Cleaning done with capacity ", cleanCapacity)
#     # update dirt value
#     dirt = getFRoomDirt()
#     print("Dirt amount on tile ", dirtPosX,dirtPosY," is " ,dirt)
...
class StandardRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        if self.room.is_position_valid( newPos):
            ...
            #print("set a new position",newPos)
        else:
            self.direction = random.randrange(0,360)
...
def run_simulation(num_robots, speed, capacity, width, height, dirt_amount, min_coverage, num_trials,
    ...
    """
    # valid data check 
    if num_robots > 0 and speed > 0 and capacity >0 and width > 0 and height > 0 \
        and 0 <= min_coverage and min_coverage <= 1.0 and num_trials > 0:
        pass
    else:
         raise ValueError
    ...
    # create testRoom 
    testRoom = room_type(width,height,dirt_amount)
    # create furnished room if the specific type was set 
    if type(room_type) == FurnishedRoom:
        room_type.add_furniture_to_room()

    testRobot = list()
    for robotNum in range(num_robots):
        testRobot.append(robot_type(testRoom,speed,capacity))
    for simNum in range(num_trials):
        stepTime = 0 
        while((testRoom.get_num_cleaned_tiles() < testRoom.get_num_tiles()) and (testRoom.get_num_cleaned_tiles()/testRoom.get_num_tiles() < min_coverage) ):
            for robotNum in range(num_robots):
                testRobot[robotNum].update_position_and_clean()
            stepTime +=1
        
        stepTimeSum += stepTime
        # initialize dirt in tiles, if simNum loop is not finished
        if simNum < num_trials-1:
            testRoom.tiles = dirt_amount
    
    return stepTimeSum//num_trials
