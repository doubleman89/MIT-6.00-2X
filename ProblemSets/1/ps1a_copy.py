...
def load_cows(filename):
    ...
    with open(filename,'r',encoding='utf-8') as file:
        ...
        for line in file:
            x= line.split(",")
            newCow = Cow(x[0],int(x[1].strip("\n")))
            cows_dict[newCow.getName()] = newCow.getWeight()
...
# for instance in Cow.instances:
#     print(instance.getName(),":",instance.getWeight())
...
def compare_cow_transport_algorithms():
    ...
    print ("brute force time :", end-start1)

    start1=time.time()
    print(greedy_cow_transport(cows))
    end = time.time()
    print ("greedy time :", end-start1)
