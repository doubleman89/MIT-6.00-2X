...
# 1 . check if needle is in sphere 
# 2 . 
...
def makeSample(numNeedles):
    inCircle = 0 

    for num in range(numNeedles):
        x = random.random()
        y = random.random()
        if checkNeedle(x,y):
            inCircle +=1    
...
def estPi(precision,numTrials):
    ...
    while sDev >= precision/1.96: 
        curEst,sDev = estimate(numNeedles,numTrials)
        print (f"For throwing {numNeedles} needles estimation of the Pi is eqaul {curEst} with standard deviation {sDev:6f}")
        numNeedles *=2
